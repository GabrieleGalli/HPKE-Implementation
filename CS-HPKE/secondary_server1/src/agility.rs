...
use hpke::{
    ...
    setup_receiver, Deserializable, HpkeError, OpModeR, OpModeS, PskBundle,
    Serializable,
};
...
impl KemAlg {
    ...
    pub fn try_from_u16(id: u16) -> Result<KemAlg, AgileHpkeError> {
        let res = match id {
            ...
            _ => return Err(AgileHpkeError::UnknownAlgIdent("KemAlg", id.into())),
        };
...
impl AgileEncappedKey {
    pub fn try_lift<Kem: KemTrait>(&self) -> Result<Kem::EncappedKey, AgileHpkeError> {
...
impl AgilePrivateKey {
    pub fn try_lift<Kem: KemTrait>(&self) -> Result<Kem::PrivateKey, AgileHpkeError> {
        Kem::PrivateKey::from_bytes(&self.privkey_bytes).map_err(|e| e.into())
    }
}

#[derive(Clone)]
pub struct AgileKeypair(pub AgilePrivateKey, pub AgilePublicKey);

impl AgileKeypair {
    pub fn try_lift<Kem: KemTrait>(&self) -> Result<(Kem::PrivateKey, Kem::PublicKey), AgileHpkeError> {
        Ok((self.0.try_lift::<Kem>()?, self.1.try_lift::<Kem>()?))
    }

    pub fn validate(&self) -> Result<(), AgileHpkeError> {
